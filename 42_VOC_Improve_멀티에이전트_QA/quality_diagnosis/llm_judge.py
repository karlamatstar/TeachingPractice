...
JUDGE_LOG_DIR = REPORTS_DIR / "logs" / "llm_judge"
...
class JudgeRunLog:
    ...
    def __init__(self, cases: list[dict]):
        ...
        self.data = {
            ...
            "case_results": [],
            "outputs": {
                "csv": str(REPORTS_DIR / "llm_judge_result.csv"),
                "markdown": str(REPORTS_DIR / "quality_score_report.md"),
            },
            "error": None,
        }
        self._write()
...
class JudgeLLM:
    ...
    def __init__(self, providers: list[tuple[str, str, object]]):
...
def make_judge_llm():
    ...
    order = [preferred, "openai" if preferred == "anthropic" else "anthropic"]
    for provider in order:
        if provider == "anthropic" and has_anthropic:
            ...
            model = os.environ.get("JUDGE_MODEL", default_model) if provider == preferred else default_model
            providers.append((provider, model, AnthropicChat(model=model, fallback_to_openai=False)))
            ...
            model = os.environ.get("JUDGE_MODEL", default_model) if provider == preferred else default_model
            providers.append((provider, model, OpenAIChat(model=model)))
    ...
    return JudgeLLM(providers), label
...
def _empty_row(cid, mode, judge_model, criteria_names, verdict, rationale, question="", total=""):
...
async def _run_judge(cases: list[dict], run_log: JudgeRunLog) -> int:
    ...
    for case in cases:
        ...
        if not case.get("judge_enabled", False):
            ...
            print(f"  {note}")
            rows.append(_empty_row(cid, mode, "-", criteria_names, "pytest 전용", note, question=case["question"]))
            save()
            continue
        ...
        if analysis is None:
            ...
            verdict = "PASS (예외처리)" if note.startswith("PASS_NO_DATA") else "미평가"
            rows.append(_empty_row(cid, mode, configured_models, criteria_names, verdict, note, question=case["question"]))
            save()
            continue
...
def _write_reports(rows, criteria_names, rubric):
    ...
    fieldnames = [
        ...
        "total", "verdict", "immediate_hold", "api_attempts", "rationale", "analysis",
    ]
