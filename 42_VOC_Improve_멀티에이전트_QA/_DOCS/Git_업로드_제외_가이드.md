...
`_OUTPUT/quality_diagnosis/report_assets/`는 Word 보고서를 만들 때 생성되는 차트 PNG 폴더다. 그래프가 최종 DOCX 내부에 이미 포함되고, 생성 스크립트로 다시 만들 수 있으므로 Git에서 제외한다.
...
| `_OUTPUT/quality_diagnosis/logs/` | pytest·Judge 실행 증적 |
