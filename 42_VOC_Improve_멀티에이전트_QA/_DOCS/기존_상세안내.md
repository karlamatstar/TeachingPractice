...
├── _OUTPUT\
│   └── quality_diagnosis\   # 테스트 및 채점 결과 보고서 저장소
...
# 이후 .env 파일을 열어 실제 API 키 값으로 교체
# OPENAI_API_KEY=sk-proj-...
# ANTHROPIC_API_KEY=sk-ant-...
```

`.env` 파일은 `.gitignore`에 등록되어 있어 커밋 대상에서 자동으로 제외됩니다. 실수로 `git add`에 포함되었는지 `git status`로 항상 확인하세요.
...
* 필요한 키 목록과 선택적 오버라이드 항목의 전체 예시는 [.env.example](../.env.example) 참고

## 5. 실행 방법

### 에이전트 실행 (서버 구동)
시스템의 정상적인 통신을 위해 6개의 에이전트를 각각 독립된 PowerShell 창에서 실행해야 합니다.
(각 창에서 `cd <프로젝트 경로>` 및 `.\.venv\Scripts\Activate.ps1` 환경 활성화 후 실행)
...
- `_OUTPUT\quality_diagnosis\pytest_result.txt`: (파이프 출력 시) 전체 pytest 실행 로그 및 통과 여부 결과
- `_OUTPUT\quality_diagnosis\llm_judge_result.csv`: 케이스별 프롬프트, 응답 및 원본 평가 데이터 상세 내역
- `_OUTPUT\quality_diagnosis\quality_score_report.md`: 품질 점수표, 케이스별 근거 상세 내용 및 최종 배포 가능 여부 판정 문서
