...
- `src/chap16/sec01/embedding_bge_m3.ipynb`

세부 실행법이 원래 문서에 있었던 프로젝트는 `_DOCS/기존_상세안내.md`에 보존했습니다. 교재형 실습은 프로젝트 루트에서 `Set-Location src` 후 각 챕터의 코드를 실행하면 상대 경로를 유지할 수 있습니다.

## 실행 환경 안내

- Python 가상환경(`.venv`)과 Jupyter Notebook 환경을 구성하고 `requirements.txt`의 로컬 모델·임베딩 모듈을 설치해야 합니다.
- 로컬 생성 모델은 Ollama와 `llama3.2:3b` 모델이 필요하며, Hugging Face 임베딩 모델은 최초 사용 시 다운로드됩니다.
