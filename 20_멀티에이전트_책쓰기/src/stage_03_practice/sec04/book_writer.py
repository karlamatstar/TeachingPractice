...
#os.environ["USER_AGENT"] = "QA-Agent/1.0"
# 현재 폴더 경로 찾기
...
class BookWriterGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("AI Book Writer")
        ...
        self._build_ui()
        self._append_text("AI 책 작성 에이전트가 준비되었습니다. 원하는 책의 주제나 요구사항을 입력해 주세요.\n")
    ...
    def _build_ui(self):
        self.chat_area = ScrolledText(self.root, wrap=tk.WORD, font=("맑은 고딕", 10), state="disabled")
        self.chat_area.pack(fill=tk.BOTH, expand=True, padx=8, pady=(8, 4))

        bottom = ttk.Frame(self.root)
        bottom.pack(fill=tk.X, padx=8, pady=(0, 8))

        self.input_box = ttk.Entry(bottom)
        self.input_box.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.input_box.bind("<Return>", self._on_send)

        send_button = ttk.Button(bottom, text="전송", command=self._on_send)
        send_button.pack(side=tk.LEFT, padx=(8, 0))

        quit_button = ttk.Button(bottom, text="종료", command=self.root.destroy)
        quit_button.pack(side=tk.LEFT, padx=(8, 0))
    ...
    def _append_messages(self):
        for msg in self.state["messages"][self.displayed_message_count:]:
            ...
            if isinstance(msg, HumanMessage):
                self._append_text(f"사용자: {content}")
            elif isinstance(msg, AIMessage):
                self._append_text(f"AI: {content}")
            else:
                self._append_text(str(content))
        self.displayed_message_count = len(self.state["messages"])

    def _set_busy(self, busy: bool):
        self.input_box.configure(state="disabled" if busy else "normal")
        self.root.title("AI Book Writer" + (" - 처리 중..." if busy else ""))
    ...
    def _on_send(self, event=None):
        ...
        if user_input.lower() in ["exit", "quit", "q"]:
            self._append_text(f"사용자: {user_input}")
            self._append_text("창을 종료합니다.")
    ...
    def _process_message(self, user_input: str):
        try:
            self.state["messages"].append(HumanMessage(user_input))
            self.state = graph.invoke(self.state)
            save_state(current_path, self.state)
            self.root.after(0, self._append_messages)
        except Exception as exc:
            self.root.after(0, lambda: self._append_text(f"[오류] {exc}"))
        finally:
