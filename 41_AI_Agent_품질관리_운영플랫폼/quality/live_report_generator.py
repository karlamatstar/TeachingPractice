"""챗봇 실시간 대화 로그(_OUTPUT/quality/live_log/conversations.jsonl) + 실시간 채점 로그
(_OUTPUT/quality/live_log/live_evaluations.jsonl)를 합쳐 별도의 실시간 품질 리포트를 만든다.
...
- 출력: _OUTPUT/quality/live_log/{ts}_live_report.md/.csv + 최신본(live_report.md/.csv) + _DOCS/live_quality_report.md
"""
import json
import shutil
import sys
...
REPORTS_DIR = PROJECT_ROOT / "_OUTPUT" / "quality"
...
def save_live_markdown_report(df: pd.DataFrame, file_path: Path) -> None:
    ...
    lines += [
        "> 배치 테스트케이스 리포트와 별개로, 실제 사용자 대화 로그(`_OUTPUT/quality/live_log/conversations.jsonl`)와 "
        "실시간 AI Judge 채점(`_OUTPUT/quality/live_log/live_evaluations.jsonl`)만을 집계한 리포트입니다. "
...
def generate_live_report(timestamp: str | None = None) -> dict:
    ...
    if not conversations:
        raise NoLiveLogsError("대화 로그(_OUTPUT/quality/live_log/conversations.jsonl)가 비어 있습니다. 먼저 챗봇과 대화하세요.")
