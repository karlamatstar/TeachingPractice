# 웹애플리케이션 자동테스트 기초

React·Cypress 프론트와 Node·Jest 백엔드의 기능·통합 테스트를 구성한 실습입니다.

## 학습 위치

- 강의 제목: AI를 활용한 웹 애플리케이션 구축
- 강의 순서: 2026-06-29 · 23일차
- 난이도: 중급

## 폴더 구성

- `src`: 실행 코드와 실습 단계
- `_DOCS`: 설명서, 계획서, 의존성 안내
- `_OUTPUT`: 실행 결과, 로그, 리포트와 과거 산출물
- `.env` 또는 `.env.example`: 필요한 경우 사용하는 환경변수 파일
- 설치 목록 파일: 프로젝트 루트 또는 `src` 하위의 `requirements.txt`, `package.json`, `pyproject.toml` 등

## 주요 실행 진입점

- `backend/index.js`
- `backend/sequelize/index.js`
- `frontend/index.html`

세부 실행법이 원래 문서에 있었던 프로젝트는 `_DOCS/기존_상세안내.md`에 보존했습니다. 교재형 실습은 프로젝트 루트에서 `Set-Location src` 후 각 챕터의 코드를 실행하면 상대 경로를 유지할 수 있습니다.

## 실행 환경 안내

- 백엔드와 프론트엔드 각각 Node.js·npm 모듈 설치가 필요하며, E2E 실행에는 npm으로 설치되는 Cypress 환경이 사용됩니다.
- 데이터 저장은 로컬 SQLite 구성을 사용할 수 있고 MongoDB 연동 기능은 별도 MongoDB가 필요합니다. 메일 기능을 사용하면 SendGrid 인증정보도 설정해야 합니다.

## 의존성 설치

- Node.js: `npm install --prefix "backend"`
- Node.js: `npm install --prefix "frontend"`

## 산출물

새로 생성되는 로그·평가표·리포트는 `_OUTPUT`에 저장하도록 정리했습니다. 단계별 원본 코드는 `src/stage_*`에서 확인할 수 있습니다.
